...
from models_imagenet import BinarizeConv2d
import copy
...
from optimizers import ProbBop, ProbBop2ndOrder
...
def parse():
    model_names = sorted(name for name in models_imagenet.__dict__
    ...
    parser.add_argument('--minlr', '--min-learning-rate', default=0, type=float,
                        metavar='minLR', help='Initial learning rate.  Will be scaled by <global batch size>/256: args.lr = args.lr*float(args.batch_size*args.world_size)/256.  A warmup schedule will also be applied over the first 5 epochs.')
    ...
    parser.add_argument('--disable_dali', default=True, action='store_true',
    ...
    parser.add_argument('--binary_opt', type=str, default='Vanilla')
...
def main():
    ...
    model_config = {}#, "binary_opt": args.binary_opt}
    ...
    num_binary_parameters=0
    num_other_parameters=0

    for pname, p in model.named_parameters():
        #logging.info(pname+": "+str(p.ndimension()))
        if '.weights' in pname:
            binary_parameters.append(p)
        elif 'fc' in pname or 'conv' in pname:
    ...
    if args.optimizer == 'sgd':
        optimizer = torch.optim.SGD([{'params':conv_parameters,'initial_lr':args.lr},
                                     {'params':other_parameters,'initial_lr':args.lr,'weight_decay':0.}],
                                    lr=args.lr, 
                                    momentum=args.momentum, 
                                    weight_decay=args.weight_decay)
    elif args.optimizer == 'adam':
        optimizer = torch.optim.Adam([{'params':conv_parameters,'initial_lr':args.lr},
                                      {'params':other_parameters,'initial_lr':args.lr,'weight_decay':0.}],
                                    lr=args.lr, 
                                    weight_decay=args.weight_decay) 
    ...
    bin_optimizer = torch.optim.Adam(binary_parameters, lr=0.001)
    logging.info('training bin_regime: %s', bin_regime)
    bin_optimizer = adjust_optimizer(bin_optimizer, 0, bin_regime)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        # train for one epoch
        [avg_train_time,train_loss, train_prec1, train_prec5] = train(train_loader, model, criterion, scaler, optimizer, epoch, bin_optimizer)
        ...
        if args.local_rank == 0:
            ...
            results.add(epoch=epoch + 1, train_loss=train_loss, val_loss=val_loss,
                    train_error1=train_prec1, val_error1=prec1,
                    train_error5=train_prec5, val_error5=prec5)
            results.save()
...
def train(train_loader, model, criterion, scaler, optimizer, epoch, bin_optimizer):
    ...
    for i, data in enumerate(data_iterator):
        ...
        if args.lr_decay == 'cos':
            adjust_learning_rate_cos(optimizer, epoch, i, train_loader_len, args.minlr)
            #adjust_learning_rate_cos_bin(bin_optimizer, epoch, i, train_loader_len)
        elif args.lr_decay == 'MSteps':
            adjust_learning_rate(optimizer, epoch, i, train_loader_len)
            #adjust_learning_rate_bin(bin_optimizer, epoch, i, train_loader_len)
        if args.test:
            if i > 10:
                break
        ...
        if args.prof >= 0: torch.cuda.nvtx.range_push("bin_optimizer.step(last_step=len(data_iterator)")
        scaler.step(bin_optimizer)
        if args.prof >= 0: torch.cuda.nvtx.range_pop()
        scaler.update()

        binarize_model2(model)
...
def adjust_learning_rate_bin(optimizer, epoch, step, len_epoch):
    ...
    for i, param_group in enumerate(optimizer.param_groups):
        param_group['sigma'] = lr

def adjust_learning_rate_cos(optimizer, epoch, step, len_epoch, minlr=0):
    # first 5 epochs for warmup
    warmup_iter = 5 * len_epoch
    current_iter = step + epoch * len_epoch
    max_iter = args.epochs * len_epoch
    lr_min = minlr
    lr = lr_min + (args.lr - lr_min) * (1 + cos(pi * (current_iter - warmup_iter) / (max_iter - warmup_iter))) / 2
    if epoch < 5:
...
def adjust_learning_rate_cos_bin(optimizer, epoch, step, len_epoch):
    # first 5 epochs for warmup
    ...
    sigma = min_sigma + (sigma - min_sigma) * (1 + cos(pi * (current_iter - warmup_iter) / (max_iter - warmup_iter))) / 2
    if epoch < 5:
        sigma = args.sigma * current_iter / warmup_iter

    for i, param_group in enumerate(optimizer.param_groups):
        param_group['sigma'] = sigma
